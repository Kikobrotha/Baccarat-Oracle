...
export default function BaccaratAssistant() {
  ...
  const layoutClasses = useMemo(() => {
    if (activeLayout === 'focus-center') {
      return { grid: 'grid grid-cols-1 xl:grid-cols-[minmax(220px,0.7fr)_minmax(540px,1.7fr)_minmax(220px,0.7fr)] gap-5 items-stretch', player: '', center: '', banker: '' };
    }
    if (activeLayout === 'wide-table') {
      return { grid: 'grid grid-cols-1 lg:grid-cols-3 gap-5 items-stretch', player: '', center: '', banker: '' };
    }
    if (activeLayout === 'mobile-stacked') {
      return { grid: 'grid grid-cols-1 gap-4 max-w-xl mx-auto', player: 'order-2', center: 'order-1', banker: 'order-3' };
    }
    return { grid: 'grid grid-cols-1 xl:grid-cols-3 gap-6 items-start', player: '', center: '', banker: '' };
  }, [activeLayout]);
  ...
  function handleDone() {
    ...
    if (recommendation === 'DO NOT PLAY') setSkipCount(prev => prev + 1);
    setPlayerCards([]);
    setBankerCards([]);
  }
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-4 md:mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <div className="w-full md:w-auto grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <select id="layoutPreset" value={selectedLayout} onChange={e => setSelectedLayout(e.target.value as LayoutPreset)} className="w-full md:min-w-56 rounded-lg border border-white/20 bg-green-900/80 px-3 py-2 text-sm font-medium shadow-md outline-none">
            ...
            <div>
              ...
              <select id="bankerMode" value={mode} onChange={e => setMode(e.target.value as BettingMode)} className="w-full md:min-w-56 rounded-lg border border-white/20 bg-green-900/80 px-3 py-2 text-sm font-medium shadow-md outline-none">
        ...
        <div className={`${layoutClasses.grid} transition-all duration-300`}>
          ...
          <section className={`rounded-2xl p-4 md:p-6 bg-black/70 text-center shadow-2xl shadow-black/30 ${layoutClasses.center}`}>
            ...
            <button onClick={handleDone} className={`mt-5 w-full bg-yellow-300 text-black font-extrabold rounded-xl shadow-lg hover:bg-yellow-200 transition ${activeLayout === 'mobile-stacked' ? 'py-4 text-xl' : isCompact ? 'py-2.5 text-base' : 'py-3.5 text-lg'}`}>
              DONE
            </button>
